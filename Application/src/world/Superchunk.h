...
struct Superchunk
{
    ...
    {
        archive(sChunk, heightmap, heightmapStartingChunk,
            twoSidedBlocks, threeSidedBlocks);
    }
    ...
    {
        std::pair<glm::vec3, glm::vec3> rayVecs = cam->CastRay(3.0f);
    ...
    void generateTree(glm::ivec2 treePos)
    {
        ...
        for (int dy = y + 2; dy < y + 6; dy++)
        {
            for (int dx = x - 2; dx <= x + 2; dx++)
            {
                ...
                {
                    if (dx != x || dz != z || dy != y + 5)
                    { // Prevent leaves from replacing trunk or appearing on the top trunk block
