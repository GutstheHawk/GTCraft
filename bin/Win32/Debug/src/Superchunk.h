...
struct Superchunk
{
    ...
    void placeBlock(Camera* cam)
    {
        ...
        if (!intersectedCubes.empty())
        {
            ...
            uint8_t blockPlaceType = REDFLOWER;

            switch (closestFace) {
            case PositiveX:
                playerChunk->setBlock(cubeChunkX + 1, cubeChunkY, cubeChunkZ, blockPlaceType);
                break;
            case NegativeX:
                playerChunk->setBlock(cubeChunkX - 1, cubeChunkY, cubeChunkZ, blockPlaceType);
                break;
            case PositiveY:
                playerChunk->setBlock(cubeChunkX, cubeChunkY + 1, cubeChunkZ, blockPlaceType);
                break;
            case NegativeY:
                playerChunk->setBlock(cubeChunkX, cubeChunkY - 1, cubeChunkZ, blockPlaceType);
                break;
            case PositiveZ:
                playerChunk->setBlock(cubeChunkX, cubeChunkY, cubeChunkZ + 1, blockPlaceType);
                break;
            case NegativeZ:
                playerChunk->setBlock(cubeChunkX, cubeChunkY, cubeChunkZ - 1, blockPlaceType);
                break;
